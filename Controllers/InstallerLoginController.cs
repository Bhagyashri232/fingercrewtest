...
namespace FingerCrew.Controllers
{
    ...
    [ApiController]
    public class InstallerLoginController : ControllerBase
    {
        InstallerLogin login = new InstallerLogin();
        [HttpPost]
        [Route("loginInstaller")]
        public IActionResult loginInstaller(loginInstallerMDetails param, [FromHeader] string authorization)
        {
            ...
            try
            {
                ...
                if (lstTokenValidation.Count > 0)
                {
                    ...
                    {
                        oresponse = login.loginInstaller(param);
                    }
                    else
                    {
            ...
            {
                CommonUtilities.FnStoreErrorLog("FingerCrewAPI", "loginInstaller_controller", ex.Message, ex.StackTrace);
                oresponse.status = "Failed";
