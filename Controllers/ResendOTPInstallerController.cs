...
namespace FingerCrew.Controllers
{
    ...
    [ApiController]
    public class ResendOTPInstallerController : ControllerBase
    {
        ResendOTPInstaller ResendOTP = new ResendOTPInstaller();
        [HttpPost]
        [Route("resendOTPInstaller")]
        public IActionResult resendOTPInstaller(resendOTPInstallerMDetails param, [FromHeader] string authorization)
        {
            ...
            try
            {
                ...
                if (lstTokenValidation.Count > 0)
                {
                    ...
                    {
                        oresponse = ResendOTP.resendInstallerOTP(param);
                    }
                    else
                    {
            ...
            {
                CommonUtilities.FnStoreErrorLog("FingerCrewAPI", "resendOTPInstaller_controller", ex.Message, ex.StackTrace);
                oresponse.status = "Failed";
