...
namespace Mob_Surveyor_Login_API.Model
{
    public class LoginMaster
    {
        DBHelper dBHelper = new DBHelper();
        public OResponse login(loginMDetails param)
        {
            ...
            try
            {
                ...
                paramList.Add(new SqlParameter("@type", "SurveyorLogin"));
                ...
                if (Regex.IsMatch(mob, pattern))
                {
                    ...
                    if (oresponse.status == "Success")
                    {
                        ...
                        paramList2.Add(new SqlParameter("@mobile_no", param.mobile_no));
                        
                        dtstatus = dBHelper.GetTableFromSP("[dbo].[mob_login_surveyor]", paramList2.ToArray());
                        foreach (DataRow dr in dtstatus.Rows)
                        {
                            oresponse.status = Convert.ToString(dr["Status"]);
                            oresponse.remarks = Convert.ToString(dr["Remarks"]);
            ...
            {
                CommonUtilities.FnStoreErrorLog("FingerCrewAPI", "login", ex.Message, ex.StackTrace);
                oresponse.status = "Failed";
