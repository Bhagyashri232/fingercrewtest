...
{
    public class SurveyorPersonalDetailsViewAndInsert
    {
        DBHelper dBHelper = new DBHelper();
        public Response addSurveyorPersonalMaster(surveyorPersonalMDetails param)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    response.remarks = "Invalid Mobile Number Format";
                }
            }
            catch (Exception ex)
            {
                CommonUtilities.FnStoreErrorLog("FingerCrewAPI", "addSurveyorPersonalMaster", ex.Message, ex.StackTrace);
                response.status = "Failed";
        ...
        public surveyorPersonalDetailResponse getSurveyorPersonalData()
        {
            surveyorPersonalDetailResponse surveyorDetailsPersonalResponse = new surveyorPersonalDetailResponse();
            displaySurveyorPersonalDetails disSurveyorDetails = new displaySurveyorPersonalDetails();
            List<displaySurveyorPersonalDetails> lstDisplayData = new List<displaySurveyorPersonalDetails>();
            DataTable dtSurveyorDetails = new DataTable();
            try
            {
                List<SqlParameter> paramList = new List<SqlParameter>();
                paramList.Add(new SqlParameter("@type", "GetAllSurveyorDetails"));
                ...
                if (dtSurveyorDetails.Rows.Count > 0)
                {
                    ...
                    {
                        surveyorDetailsPersonalResponse.status = Convert.ToString(dtSurveyorDetails.Rows[0]["Status"]);
                        ...
                        foreach (DataRow dr in dtSurveyorDetails.Rows)
                        {
                            ...
                            disSurveyorDetails = new displaySurveyorPersonalDetails();

                        }
                        surveyorDetailsPersonalResponse.remarks = " ";
                        surveyorDetailsPersonalResponse.getData = lstDisplayData;

                    }
                    else
                    {
                        surveyorDetailsPersonalResponse.status = Convert.ToString(dtSurveyorDetails.Rows[0]["Status"]);
                        surveyorDetailsPersonalResponse.remarks = Convert.ToString(dtSurveyorDetails.Rows[0]["Remarks"]);
                    }
            ...
            {
                CommonUtilities.FnStoreErrorLog("FingerCrewAPI", "getSurveyorPersonalData", ex.Message, ex.StackTrace);
            }

            return surveyorDetailsPersonalResponse;
        }
    }
    ...
    public class surveyorPersonalMDetails
    {
    ...
    public class surveyorPersonalDetailsParam
    {
        public string? surveyor_master_id { get; set; }
    }

    public class surveyorPersonalDetailResponse
    {
        public string? status { get; set; }
        public string? remarks { get; set; }
        public List<displaySurveyorPersonalDetails>? getData { get; set; }
    }

    public class displaySurveyorPersonalDetails
    {
        public string? surveyor_master_id { get; set; }
        public string? salutation { get; set; }
        public string? surveyor_name { get; set; }
